...
%__________________________________________________________________________
    ...
    LA       = params.LA;      % [micro m]
    LM       = params.LM;      % [micro m]
    LB       = params.LB;      % [micro m]
    n_RU     = params.n_RU;    % [-]
    Lsmooth  = params.Lsmooth; % [micro m]
    % RU parameters
    Kon      = params.Kon;     % [micro M^-1 * s^-1]
    Koff     = params.Koff;    % [s^-1]
    Kbasic   = params.Kbasic;  % [s^-1]
    Q0       = params.Q0;      % [-]
    SLQ      = params.SLQ;     % [micro m]
    alphaQ   = params.alphaQ;  % [micro m^-1]
    mu       = params.mu;      % [-]
    gamma    = params.gamma;   % [-]
    TaMax    = params.TaMax;   % [kPa]
