...
class model_RDQ18:
    ...
    def __init__(self, params = '../params/params_RDQ18.json'):
        ...
        self.LA       = float(params['LA'])      # [micro m]
        self.LM       = float(params['LM'])      # [micro m]
        self.LB       = float(params['LB'])      # [micro m]
        self.n_RU     =   int(params['n_RU'])    # [-]
        self.Lsmooth  = float(params['Lsmooth']) # [micro m]
        # RU parameters
        self.Kon      = float(params['Kon'])     # [micro M^-1 * s^-1]
        self.Koff     = float(params['Koff'])    # [s^-1]
        self.Kbasic   = float(params['Kbasic'])  # [s^-1]
        self.Q0       = float(params['Q0'])      # [-]
        self.SLQ      = float(params['SLQ'])     # [micro m]
        self.alphaQ   = float(params['alphaQ'])  # [micro m^-1]
        self.mu       = float(params['mu'])      # [-]
        self.gamma    = float(params['gamma'])   # [-]
        self.TaMax    = float(params['TaMax'])   # [kPa]
